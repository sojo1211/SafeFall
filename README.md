...
---
창업 아이디어 경진 대회에서 진행한 '낙상 사고 예측 프로젝트' 당시, 기존의 낙상 탐지 연구들이 92% 이상의 높은 정확도를 보임에도 불구하고, 대부분 사고 발생 '후'를 감지하는 한계가 있다는 점에 주목했습니다. 이를 해결하기 위해 사고 자체를 미리 방지하는 예측 시스템을 제안했습니다,

제 역할은 다중 센서 데이터인 거리, 심박 데이터 등을 학습할 수 있는 방식으로 만들기 위해 라벨링,전처리, 실험, 품질 검증 역할을 맡았습니다.
모든 사용자의 거리와  심박 데이터를 CORR()로 상관분석을 계산한 뒤, 히트맵으로 시각화했습니다, 낙상 직전의 전조 증상은 사용자마다 패턴이 너무 다양해 고정된 임계값으로 라벨링 하는 데 한계가 있었습니다.
이를 해결하기 위해 비지도 학습인 K-Means를 도입했습니다. 상관계수와 센서 데이터의 특징점을 기반으로 군집화를 수행하여, 사람이 정의하기 어려운 전조 증상의 경계 지점을 알고리즘으로 도출했습니다.

그 결과, 최우수상을 거두며, 몸부림 등 위험 징후 포착에서 83%의 정확도와 약 82%의 F1-Score를 달성하며 사후 감지가 아닌 '사전 예방'의 실효성을 확인했습니다.
...
논문 출처:Wang, D., & Li, Z. (2023). Comparison of four machine learning algorithms for a pre-impact fall detection system. Medical & Biological Engineering & Computing, 61(8), 1961–1974. https://doi.org/10.1007/s11517-023-02853-8

기존 낙상 탐지(Fall Detection) 연구들은 정확도 약 92–99% 수준의 높은 성능을 보이지만, 대부분 낙상 발생 이후를 인지하는 사후 대응 방식이라는 한계를 가진다.
본 프로젝트는 이러한 한계를 극복하고, 낙상 이전의 위험 신호를 사전에 포착하여 근본적인 재해 사고를 예방하기 위한 낙상 예측(Fall Prediction)을 목표로 한다
