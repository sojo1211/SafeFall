...
제 역할은 다중 센서 데이터인 거리, 심박 데이터 등을 학습할 수 있는 방식으로 만들기 위해 라벨링,전처리, 품질 검증 역할을 맡았습니다.
모든 사용자의 거리와 심박 데이터를, 상관분석을 계산한 뒤, 히트맵으로 시각화했습니다, 낙상 직전의 전조 증상은 사용자마다 패턴이 너무 다양해 고정된 임곗값으로 라벨링 하는 데 한계가 있었습니다.
